...
interface StructuralCallImpl extends StructuralCall {
  ...
  static MethodHandle findMethodHandle(Lookup lookup, String name, MethodType type) {
    ...
    MethodHandle mh = new InliningCacheCallSite(type.insertParameterTypes(0, Object.class), lookup, name).dynamicInvoker();
  ...
  class InliningCacheCallSite extends MutableCallSite {
    ...
    InliningCacheCallSite(MethodType type, Lookup lookup, String name) {
      ...
      this.lookup = lookup;
      this.name = name;
      setTarget(MethodHandles.foldArguments(MethodHandles.exactInvoker(type), FALLBACK.bindTo(this)));
    }
    ...
    private MethodHandle fallback(Object receiver) {
      ...
      MethodHandle guard = MethodHandles.guardWithTest(TYPECHECK.bindTo(receiverClass),
          target,
    ...
    private static MethodHandle findTarget(Lookup lookup, Class<?> receiverClass, String name, MethodType parameterType) throws IllegalAccessException, NoSuchMethodException {
