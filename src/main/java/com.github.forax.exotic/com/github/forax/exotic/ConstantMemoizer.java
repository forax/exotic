...
/**
 *  Allow to see the return value of a function as a constant in case there are few possible pairs of key/value.
 *  
 ...
 *  </pre>
 */
public class ConstantMemoizer {
  private ConstantMemoizer() {
    throw new AssertionError();
  }
  
  /**
   * Return a function that returns a constant value (for the Virtual Machine) for each key taken as argument.
   * The value corresponding to a key is calculated by calling the {@code function} once by key
   * and then cached in a code similar to a cascade of {@code if equals else}.
   * 
   * To find if a key was previously seen or not, {@link Object#equals(Object)} will be called to compare
   * the actual key with possibly all the keys already seen, so if there are a lot of different keys,
   * the performance in the worst case is like a linear search i.e. O(number of seen keys). 
   * 
   * @param <K> type of the keys.
   * @param <V> type of the values.
   * @param function a function that takes a non null key as argument and return a non null value.
   * @param keyClass the class of the key, if it's a primitive type, the key value will be boxed before calling
   *        the {@code function}.
   * @param valueClass the class of the value, if it's a primitive type, the value will be boxed at each call.
   * @return a function the function getting the value for a specific key.
   * @throws NullPointerException if the {@code function}, the {@code keyClass} or the {@code valueClass} is null,
   *         or if the function key or the function value is null.
   * @throws ClassCastException if the function key or the function value types doesn't match the {@code keyClass} or
   *         the {@code valueClass}. 
   */
  public static <K, V> Function<K, V> memoizer(Function<? super K, ? extends V> function, Class<K> keyClass, Class<V> valueClass) {
    Objects.requireNonNull(function);
    Objects.requireNonNull(keyClass);
    Objects.requireNonNull(valueClass);
    MethodHandle mh = new InliningCacheCallSite<>(methodType(valueClass, keyClass), function)
        .dynamicInvoker()
  ...
  private static class InliningCacheCallSite<K,V> extends MutableCallSite {
    ...
    private MethodHandle fallback(K key) {
      ...
      setTarget(guardWithTest(EQUALS.bindTo(key).asType(methodType(boolean.class, keyClass)),
          target,
